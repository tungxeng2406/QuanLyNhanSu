...
| DB-SCH-006 | SCHEMA | Kiem tra cac index da tao | DB da tao bang | 1) Query index metadata | Index full_name/department/status/hire_date/email/phone ton tai | Medium | ⏳ |
...
| DB-CON-004 | CONSTRAINT | Vi pham not null status | none | 1) Insert status null | null status | Loi not null | High | ⏳ |
...
| DB-CON-006 | CONSTRAINT | Status ngoai tap cho phep (neu check bat) | check da cau hinh | 1) Insert status=UNKNOWN | invalid status | Reject hoac duoc chan o app-level theo quy dinh | Medium | ⏳ |
